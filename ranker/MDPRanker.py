...
class MDPRanker(AbstractRanker):
    def __init__(self,
        ...
        cross_entropy_pos = tf.reduce_sum(-tf.log(tf.clip_by_value(prob,1e-10,1.0))* tf.one_hot(position, doc_length),
                                      axis=1)
        grads_vars_pos = opt.compute_gradients(cross_entropy_pos)
        train_step_pos = opt.apply_gradients(grads_vars_pos)
        #
        cross_entropy_neg = tf.reduce_sum(-tf.log(tf.clip_by_value(tf.ones([1, doc_length])-prob, 1e-10, 1.0)) * tf.one_hot(position, doc_length),
                                          axis=1)
        grads_vars_neg = opt.compute_gradients(cross_entropy_neg)
        train_step_neg = opt.apply_gradients(grads_vars_neg)
