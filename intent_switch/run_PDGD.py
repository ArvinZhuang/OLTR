...
def run(train_set, test_set, ranker, num_interation, click_model):
    qrel_dic0 = read_intent_qrel("0.txt")
    ...
    qrel_dic5 = read_intent_qrel("5.txt")

    train_set.update_relevance_label(qrel_dic0)
    all_result = ranker.get_all_query_result_list(train_set)
    ndcg = evl_tool.average_ndcg_at_k(train_set, all_result, 10)
    print(ndcg)

    train_set.update_relevance_label(qrel_dic1)
    all_result = ranker.get_all_query_result_list(train_set)
    ndcg = evl_tool.average_ndcg_at_k(train_set, all_result, 10)
    print(ndcg)

    train_set.update_relevance_label(qrel_dic2)
    all_result = ranker.get_all_query_result_list(train_set)
    ndcg = evl_tool.average_ndcg_at_k(train_set, all_result, 10)
    print(ndcg)

    train_set.update_relevance_label(qrel_dic3)
    all_result = ranker.get_all_query_result_list(train_set)
    ndcg = evl_tool.average_ndcg_at_k(train_set, all_result, 10)
    print(ndcg)

    train_set.update_relevance_label(qrel_dic4)
    all_result = ranker.get_all_query_result_list(train_set)
    ndcg = evl_tool.average_ndcg_at_k(train_set, all_result, 10)
    print(ndcg)

    train_set.update_relevance_label(qrel_dic5)
    all_result = ranker.get_all_query_result_list(train_set)
    ndcg = evl_tool.average_ndcg_at_k(train_set, all_result, 1000)
    print(train_set.get_relevance_docids_by_query("6"))
    print(ndcg)
